...
class DDQN_Agent:
    ...
    def test(self):
        ...
        frameSize = (next_state_image.shape[1], next_state_image.shape[0])
        video = cv2.VideoWriter("videos\\test_video_episode_{}.avi".format(self.episode), cv2.VideoWriter_fourcc(*'DIVX'), 2, frameSize)
        ...
        while True:
            ...
            if done:
                # Release the video writer and end the test
                video.release()
                end = time.time()
                stopWatch = end - start
